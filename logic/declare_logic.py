...
def declare_logic():
    ...
    # Counts all accesses made by a user and stores it in User.access_count.
    Rule.count(derive=User.access_count, as_count_of=Access)

    # Counts all invitations received by a user and stores it in User.invitation_count.
    Rule.count(derive=User.invitation_count, as_count_of=Invitation)

    # Counts all users belonging to a group and stores it in Group.user_count.
    Rule.count(derive=Group.user_count, as_count_of=User)

    # Counts all users associated with a profile and stores it in Profile.user_count.
    Rule.count(derive=Profile.user_count, as_count_of=User)

    # Ensure that invitation start and end dates are within access timings.
    Rule.constraint(validate=Invitation, as_condition=lambda row: row.start_date >= access_start_date and row.end_date <= access_end_date, error_msg='Invitation dates must be within access schedules.')
